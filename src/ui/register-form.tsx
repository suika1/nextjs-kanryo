...
export default function RegisterForm() {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const confirm = (form.querySelector('input[name="confirmPassword"]') as HTMLInputElement)
      ?.value;
    if (pwd !== confirm) {
    ...
    <form action={formAction} onSubmit={handleSubmit} className="space-y-3">
      <div className="flex-1 rounded-lg bg-gray-200 px-6 pt-8 pb-4">
        <div className="w-full">
          <div>
            <label className="mt-5 mb-3 block text-xs font-medium text-gray-900" htmlFor="name">
              Имя
            </label>
            <input
        ...
        >
          {(actionState?.error || localError) && (
            <>
              ...
              <p className="text-sm text-red-500">{localError ?? actionState?.error}</p>
            </>
          )}
        </div>
      </div>
